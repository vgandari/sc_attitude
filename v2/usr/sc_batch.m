function [tspan, IC, file_name, region, label_x_name] = sc_batch(testcase)
...
% sc_attitude), simOptions will not run and the test cases defined here may be
...
% -------------------------------------------------------------------------
    ...
    % Initial Orientation (q must be row vwctor, C must be row operator
    C0 = eye(3, 3);
...
%     end
    ...
    file_name = horzcat(file_name, '_r', num2str(region));
